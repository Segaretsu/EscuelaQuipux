 ...
 * @author Sebas
 */
public class Pnl_consultarProfesor extends JPanel implements ActionListener{
    ...
    String[] porBusquedaProfesor = {"idProfesor","nombre","apellido"};
    String[] mostrarProfesor = {"Codigo", "Nombre", "Apellido"};
    
    ProfesorManagerImpl managerProfesor =new ProfesorManagerImpl();
    
    public Pnl_consultarProfesor(Control_Instancias instancia, int width,int height){
        this.width=width;
        this.height=height;
        pp=instancia;
        
        lbl_titulo =  new JLabelArje();
            lbl_titulo.setTexto("Sección: Consultar - Profesor.", Fonts.Lato, 1, 14, Colores.pantone276C, 0);
            this.add(lbl_titulo);
        ...
        cbx_consultar=new JComboboxArje(mostrarProfesor);
    ...
    public void actionPerformed(ActionEvent ae) {
        if(ae.getSource()==btn_buscar){
            ...
            lbl_informacion.setText(managerProfesor.generarTablaDeConsulta(campo, texto));
        }
        if(ae.getSource()==btn_todo){
            
            lbl_informacion.setText(managerProfesor.generarTablaDeConsulta());
